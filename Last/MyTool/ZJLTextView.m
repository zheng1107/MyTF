...
#import "ZJLTextView.h"

@implementation ZJLTextView
...
- (void)setPlaceHolder:(NSString *)placeHolder {
    ...
    NSUInteger maxChars = [ZJLTextView maxCharactersPerLine];
...
- (NSUInteger)numberOfLinesOfText {
    return [ZJLTextView numberOfLinesForMessage:self.text];
}
...
+ (NSUInteger)numberOfLinesForMessage:(NSString *)text {
    return (text.length / [ZJLTextView maxCharactersPerLine]) + 1;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if([self.text length] == 0 && self.placeHolder) {
        ...
        if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_7_0) {
            ...
            [self.placeHolder drawInRect:placeHolderRect withAttributes:@{ NSFontAttributeName : self.font, NSForegroundColorAttributeName : self.placeHolderTextColor,NSParagraphStyleAttributeName : paragraphStyle }];
        }
        else {
